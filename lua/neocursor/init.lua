...
local function normalize_hints(v)
  if v == false then
    return { edit = false, prediction = false }
  end
...
local function log_refresh()
  ...
  if s and vim.api.nvim_buf_is_valid(s.bufnr) then
    if vim.api.nvim_get_current_buf() ~= s.bufnr then
      ...
      local at = (s.end0_excl <= s.start0) and (row0 == s.start0) or (row0 >= s.start0 and row0 < s.end0_excl)
  ...
  local phase
  if s then
    phase = state.queue and "CHAINING" or "SHOWING"
  elseif state.timer then
    phase = "DEBOUNCING"
  elseif state.awaiting then
    phase = "IN-FLIGHT"
  else
    phase = "IDLE"
  end
  ...
  local lines = {
    ...
    ("  suggest   %-16s  debounce %sms · timer %s"):format(
      sugg,
      c.debounce or "?",
      state.timer and "ticking" or "idle"
    ),
    ("  chain     %-16s  heur %d · excl %d · fused %s"):format(
      chain,
      c.heuristics and #c.heuristics or 0,
    ...
    ("  err %s · suppress %s · dismissed %d/%s%s"):format(
      ...
      tostring(state.last_suppressed or "none"),
      state.dismissed,
      tostring(c.max_cleared or 20),
...
local function apply_config(cfg)
  if type(cfg.debounce) == "number" and cfg.debounce > 0 then
    state.cfg.debounce = cfg.debounce
  end
  if type(cfg.exclude_patterns) == "table" then
    state.cfg.exclude_patterns = cfg.exclude_patterns
  end
  ...
  if type(cfg.max_cleared) == "number" then
    state.cfg.max_cleared = cfg.max_cleared
  end
  if type(cfg.is_fused) == "boolean" then
    state.cfg.is_fused = cfg.is_fused
  end
  log(
    ("CONFIG  debounce=%sms heuristics=%d excludes=%d"):format(
      state.cfg.debounce,
      #state.cfg.heuristics,
      #state.cfg.exclude_patterns
    )
  )
end
...
local function show_edit(edit)
  ...
  if mode == "inline" then
    ...
    local label = hints().edit and ((at and "<Tab> accept" or "<Tab> jump") .. " · <Esc> dismiss") or nil
...
local function advance_after_apply(applied)
  ...
  while q.idx <= #q.list do
    if show_edit(q.list[q.idx]) then
      return
    end
...
local function pred_key(p)
  return (p.path or "") .. ":" .. tostring(p.line)
end
...
local function record_pred_reject(p)
  ...
  if n > PRED_REJECT_MAX and okey then
    state.pred_rejects[okey] = nil
  end
end
...
local function pred_recently_rejected(p)
  local now = os.time() * 1000
  for k, v in pairs(state.pred_rejects) do
    if now - v.ts > PRED_REJECT_TTL then
      state.pred_rejects[k] = nil
    end
  end
  local r = state.pred_rejects[pred_key(p)]
...
local function reject_suggestion()
  ...
  log(
    ("DISMISS L%d  (rejected ×%d · %d/%s before mute)"):format(
      s.start0 + 1,
      state.rejects[key],
      state.dismissed,
...
local function show_prediction()
  ...
  if (not p.path) or p.path == rel then
    ...
    if row1 == vim.api.nvim_win_get_cursor(0)[1] then
      return false
    end
    if paint then
...
local function render_result(res)
  ...
  local suppressed = heuristics.should_suppress(state.cfg.heuristics, {
    bufnr = bufnr,
    start0 = first.start0,
    end0_excl = first.end0_excl,
    lines = first.lines,
    recently_removed = recently_removed(path),
    rejections = state.rejects,
    key = reject_key(path, first),
    hard_reject = state.cfg.reject_hard,
  })
  if suppressed then
  ...
  while state.queue.idx <= #list do
    if show_edit(list[state.queue.idx]) then
      return
    end
    state.queue.idx = state.queue.idx + 1
  end
  state.queue = nil
  if show_prediction() then
    return
  end -- all edits stale, but a jump remains
...
local function result_summary(res)
  ...
  local pred = res.prediction and ("  pred=" .. tostring(res.prediction.path) .. ":" .. tostring(res.prediction.line))
    or ""
...
local function handle_result(res)
  ...
  end -- the current flight resolved
  ...
  if res.id ~= state.seq then
    ...
    if
      ...
      and #res.edits > 0
      and rt.bufnr == cb
      and vim.api.nvim_buf_get_changedtick(cb) == rt.tick
    then
      log(("RESCUE  #%s  (late, buffer unchanged)  %s"):format(tostring(res.id), result_summary(res)))
      vim.schedule(function()
        if vim.api.nvim_get_current_buf() == cb and vim.api.nvim_buf_get_changedtick(cb) == rt.tick then
          render_result(res)
        end
      end)
      return
...
local function collect_additional_files(cur_buf)
  ...
  local function push(b, is_open, top, bot, ts)
    ...
    if not path or excluded(path) then
      return
    end
  ...
  for _, d in ipairs(diags) do
    ...
    errors[#errors + 1] = {
      ...
      range = {
        sl = d.lnum or 0,
        sc = d.col or 0,
        el = d.end_lnum or d.lnum or 0,
        ec = d.end_col or d.col or 0,
      },
    }
  end
...
local function commit_diff(buf)
  ...
  if not traj then
    traj = {}
    state.dtraj[path] = traj
  end
  traj[#traj + 1] = { diff = d, ts = os.time() * 1000 }
  while #traj > MAX_TRAJ do
    table.remove(traj, 1)
  end
...
local function collect_file_diff_histories(cur_buf, cur_path)
  ...
  for path, traj in pairs(state.dtraj) do
    if #traj > 0 then
      local diffs, ts = {}, {}
      for _, e in ipairs(traj) do
        diffs[#diffs + 1] = e.diff
        ts[#ts + 1] = e.ts
      end
...
local function send_request()
  ...
  state.awaiting = true
  log(
    ("REQ     #%d  %s %d:%d  ctx=%d diffs=%d lint=%d"):format(
      state.seq,
      path,
      cur[1] - 1,
      cur[2],
...
local function should_attach(bufnr)
  if vim.bo[bufnr].buftype ~= "" then
    return false
  end -- skip prompts/terminals/nofile
...
local function schedule_request(keep)
  if not should_attach(vim.api.nvim_get_current_buf()) then
    return
  end
  if not keep then
    clear_suggestion()
...
local function try_retain()
  ...
  if s.end0_excl > lc or s.end0_excl <= s.start0 then
    return false
  end
  ...
  if state.suggestion.mode ~= "inline" then
    return false
  end -- deviated; drop it
  return true
end
  ...
  if not ok then
    ...
    if msg:find("E565") or msg:find("E523") or msg:find("textlock") then
      vim.schedule(function()
        ...
        if not ok2 then
          log("ERR     apply failed: " .. tostring(err2))
        end
      end)
    else
...
local function tab_should_indent(s)
  if state.cfg.is_fused ~= false then
    return false
  end
  if s.mode == "inline" then
    return false
  end
...
function M._log_lines()
  return vim.deepcopy(state.log)
end

-- Dismiss without leaving insert mode. <Esc> does the same thing and then exits insert; this is the variant for when you want to keep typing. <C-]> is the
...
function M.dismiss()
  ...
  if not reject_suggestion() then
    reject_prediction(bufnr)
  end
...
function M.suggest()
  send_request()
end -- manual trigger (:NeocursorSuggest)
...
function M.setup(opts)
  ...
  vim.api.nvim_create_autocmd({ "TextChangedI", "CursorMovedI" }, {
    ...
    callback = function(args)
      ...
      if seen and seen.buf == buf and seen.tick == tick and seen.row == cur[1] and seen.col == cur[2] then
        return
      end
  ...
  vim.api.nvim_create_autocmd("InsertEnter", {
    ...
    callback = function()
      ...
      if rejected_too_many() then
        return
      end -- Cursor gates its EditorChange trigger the same way
...
vim.api.nvim_create_autocmd("ModeChanged", {
  ...
  callback = function(args)
    ...
    -- [dizzi patch] Don't interfere with visual mode transitions at all
    if old_mode:match("^[vV]") or new_mode:match("^[vV]") then
      return -- Let visual mode transitions happen naturally
    end
  ...
  vim.api.nvim_create_user_command("NeocursorDebug", function()
    ...
    local lines = {
      ...
      "dismissed   : "
        .. state.dismissed
        .. "/"
        .. tostring(state.cfg.max_cleared or 20)
